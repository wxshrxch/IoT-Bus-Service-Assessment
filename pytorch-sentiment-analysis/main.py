...
def main(config):
    if config.mode == 'train':
        ...
        # for cnn
        test_data = pad_sentence(test_data, config.filter_sizes[-1])
        cnn_iter, cnn_idx = make_iter(config.batch_size, config.mode, test_data=test_data)

        if config.ensemble:
            trainer = Trainer(config, pad_idx, cnn_idx, test_iter=test_iter, cnn_iter=cnn_iter)
            trainer.ensemble_inference()
        else:
...
if __name__ == '__main__':
    ...
    #모델 이름 지정
    parser.add_argument('--save_model', type=str, default='rnn_model.pt')
    ...
    # 앙상블 전용 터미널 명령어
    # python main.py --model cnn  --ensemble True --mode test
